                ...
                {albumList[id].map((song, index) => (
                    <li 
                        ...
                        className={`
                            ...
                            ${songsList === albumList[id] && currentSong === index ? 'bg-neutral-800 text-white' : ''}
                        `}
                        onClick={(e) => {
                            if (songsList !== albumList[id] && e.currentTarget.id == musicPlayer.current.id) {
                                setReplay()
                            }
                            setSongsArray(albumList[id])
                            setCurrentSong(index)
                            setPlaying()
                            toggleDummy()
                        }}
                    >
                        <div className='w-10 pr-2'>{songsList === albumList[id] && currentSong === index ? <RiVolumeUpLine className='text-lg'/> : (index + 1 < 10 ? `0${index + 1}` : index + 1)}</div>
